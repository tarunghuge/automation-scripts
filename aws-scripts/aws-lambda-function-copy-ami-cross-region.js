...
def copy_latest_image():
    images = source_image_resource.images.filter(Owners=["648445750574"]) # Specify your AWS account owner id in place of "XXXXX" at all the places in this script
    
    #Retention days in DR region, its for 7 days.
    ...
    for image in images:
	    ...
	    #To Copy previous day images
            ...
            if not dest_image_client.describe_images(Owners=['648445750574',],Filters=[{'Name':'name','Values':[image.name]}])['Images']:
            #if not dest_image_client.describe_images(Owners=['XXXXX',])['Images']:
            
                print ("Copying Image {name} - {id} to Mumbai".format(name=image.name,id=image.id))
                ...
                print ("New Image Id {new_id} for Singapore Image {name} - {id}".format(new_id=new_ami,name=image.name,id=image.id))
                ...
                print ("Image {name} - {id} already present in Mumbai Region".format(name=image.name,id=image.id))
